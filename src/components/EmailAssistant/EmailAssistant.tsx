// src/features/EmailAssistant/EmailAssistant.tsx
...
const EmailAssistant = () => {
  ...
  // Chat state
  const [askInput, setAskInput] = useState('') // 🔒 never overwritten by "thinking"
  const [thinkingLabel, setThinkingLabel] = useState('') // 🆕 separate ephemeral label
  ...
  function openSocket(): Promise<WebSocket> {
    return new Promise((resolve, reject) => {
      ...
      // onmessage/onclose are (re)assigned per request in sendPrompt
    })
  }
  ...
  const sendPrompt = async (prompt: string) => {
    // If already streaming, treat as cancel
    if (isTyping) {
      try {
        wsRef.current?.close(4001, 'client cancel')
      } catch {}
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        ws.onclose = (ev) => {
          // If closed before done and nothing streamed, surface error (unless aborted)
      ...
      if (aborted) {
        setAiResponse((prev) => (prev ? prev : 'Request cancelled.'))
      } else {
  ...
  const handleRegenerate = () => {
    ...
    // Keep input as-is; just resend the last prompt
  ...
  const handleAsk = () => {
    // If we're currently streaming, treat click/Enter as "cancel"
    if (isTyping) {
      sendPrompt('') // triggers the cancel path in sendPrompt
      return
    }
  ...
  useEffect(() => {
    const fetchPrompts = async () => {
      try {
        ...
        setPrompts(Array.isArray(data.prompts) ? data.prompts.slice(0, 6) : defaultPrompts.slice(0, 6))
      } catch (error) {
        console.error('Failed to fetch prompts:', error)
        setPrompts(defaultPrompts.slice(0, 6))
      }
    }
    fetchPrompts()
  }, [])
        ...
        <div className={styles.assistantPanel}>
          ...
          {aiExpanded && (
            <div className={styles.aiCore}>
              <div className={styles.promptBar}>
                ...
                {isTyping ? (
                  <CircleStop
                    ...
                    color="#0073ff"
                    size={22}
                    onClick={handleAsk} // acts as cancel while streaming
                  />
                ) : (
                  <img
                    src={playIcon}
              ...
              {/* thinking indicator lives separate from input value */}
              {isTyping && thinkingLabel && (
                <div className={styles.thinkingLabel} aria-live="polite">
                  {thinkingLabel}
                </div>
              )}
              ...
              <div className={styles.promptSuggestions}>
                {prompts.map((text, index) => (
                  <button key={index} onClick={() => setAskInput(text)} title={text}>
                    {text}
                  </button>
                ))}
              </div>
              ...
              <div className={styles.aiResponse}>
                <div className={styles.responseContent} aria-live="polite">
